...
`;function v({field:e,label:t,helper:n,items:r,formatter:i}){let a=r.find(t=>t.id===g.profile[e]),o=g.openPicker===e,s=`${e}-options`,c=r.map(t=>`
    <button class="select-option ${t.id===g.profile[e]?`is-selected`:``}" type="button" aria-pressed="${t.id===g.profile[e]}" data-action="pick-option" data-field="${e}" data-value="${t.id}">
  ...
  `).join(``);return`
    <div class="field-group picker-field">
      ...
      ${o?`
        <button class="picker-scrim" type="button" data-action="close-picker" aria-label="关闭${t}选择器"></button>
        <section class="select-sheet" id="${s}" role="dialog" aria-modal="true" aria-labelledby="${e}-sheet-title">
          <header class="select-sheet-header">
            <div>
              <p>选择资料</p>
              <h2 id="${e}-sheet-title">${t}</h2>
            </div>
            <button class="sheet-close" type="button" data-action="close-picker">完成</button>
          </header>
          <div class="select-options" role="group" aria-label="${t}选项">${c}</div>
        </section>
      `:``}
      <small>${n}</small>
    </div>
  ...
  `,`length`)}function S(){let e=g.questions,t=e[g.questionIndex],n=g.answers[g.questionIndex];return _(`
  ...
  `,`analyzing`)}function w(e,t){return e.map((e,n)=>`
  ...
  `).join(``)}function T(){let{result:n}=g,r=e.find(e=>e.id===g.profile.zodiac),i=t.find(e=>e.id===g.profile.mbti),a=n.preferences.slice(0,3).map(e=>`<span class="preference-tag">${{direct:`坦诚沟通`,reassurance:`情绪回应`,independent:`独立空间`,reliable:`稳定行动`}[e]}</span>`).join(``);return _(`
      ...
      <section class="matches-section">
        ...
        <div class="match-columns">
          <div class="match-group">
            <h3>星座传统配对参考</h3>
            ${w(n.topZodiacs,`传统元素配对提示`)}
          </div>
          <div class="match-group">
            <h3>MBTI 互动偏好参考</h3>
            ${w(n.topMbti,`本次答题的偏好模型`)}
          </div>
        </div>
      </section>
  ...
  `,`result`)}function E(){h.innerHTML={welcome:y,profile:b,length:x,quiz:S,analyzing:C,result:T}[g.screen]()}function D(){g.screen=`welcome`,g.profile={zodiac:``,mbti:``},g.questionCount=12,g.questionIndex=0,g.answers=[],g.validationError=``,g.result=null,g.questions=[],g.openPicker=null}function O(){g.screen=`analyzing`,E(),window.setTimeout(()=>{g.result=m(g.profile,g.answers),g.screen=`result`,E()},720)}h.addEventListener(`click`,e=>{let t=e.target.closest(`[data-action]`);if(!t)return;let{action:n}=t.dataset;if(n===`home`||n===`restart`){D(),E();return}if(n===`toggle-picker`){g.openPicker=g.openPicker===t.dataset.field?null:t.dataset.field,E();return}if(n===`close-picker`){g.openPicker=null,E();return}if(n===`pick-option`){g.profile[t.dataset.field]=t.dataset.value,g.openPicker=null,g.validationError=``,E();return}if(n===`to-profile`){g.screen=`profile`,g.openPicker=null,E();return}if(n===`to-length`){!g.profile.zodiac||!g.profile.mbti?g.validationError=`请选择你的星座和 MBTI 后再继续。`:(g.screen=`length`,g.openPicker=null),E();return}if(n===`choose-length`){g.questionCount=Number(t.dataset.count),g.questionIndex=0,g.answers=[],g.questions=r(g.questionCount),g.screen=`quiz`,E();return}if(n===`previous`&&g.questionIndex>0){--g.questionIndex,E();return}if(n===`answer`){if(g.answers[g.questionIndex]=t.dataset.value,g.questionIndex===g.questionCount-1){O();return}g.questionIndex+=1,E()}}),E();